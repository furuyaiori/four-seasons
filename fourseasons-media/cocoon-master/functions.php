...
function _my_tinymce($initArray) {
  $style_formats = array(
      array(
          'title' => 'ボタン・テスト',
          'block' => 'a',
          'classes' => 'button-editor',
          'attributes' => array('href' => 'リンクURL'),
      ),
  array(
    'title' => '角丸',
    'block' => 'div',
...
// この関数をTinyMCEの初期化時に適用
add_filter('tiny_mce_before_init', '_my_tinymce');
function custom_editor_styles() {
add_editor_style('css2/editor-style.css');
}

add_action('admin_init', 'custom_editor_styles');

function my_custom_box_shortcode($atts, $content = null) {
    // デフォルト属性値
    $atts = shortcode_atts(
        array(
            'title' => 'ここにタイトル', // タイトルのデフォルト値
        ),
        $atts,
        'box'
    );
    ...
    // コンテンツ（ショートコードの本文）をdo_shortcodeで処理
    $content = do_shortcode($content); // HTMLエスケープを削除

    // カスタムボックスのHTMLを組み立て
    return "<div class=\"box26\"><span class=\"box-title\">{$title}</span><p>{$content}</p></div>";
}
add_shortcode('box', 'my_custom_box_shortcode');
